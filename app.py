...
def predict_next_day_price(date, comment, model):
    ...
    if date in df['Date'].values:
        ...
        next_day_index = date_index + 1

        if not comment.strip():
            if next_day_index < len(df):
                return df.iloc[next_day_index]['Close'] 
            else:
    ...
    sentiment_score = get_sentiment_score(comment) if comment.strip() else 0  

    if last_sequence.shape[1] == 28:
        last_sequence = np.hstack((last_sequence, np.full((sequence_length, 1), sentiment_score)))
    elif last_sequence.shape[1] == 29:
